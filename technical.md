...
* **AWS Bedrock Configuration:** The system initializes clients for **Amazon Titan Embeddings v2** (for cost-optimized vectorization at $0.02 per 1M tokens) and **Claude 3.5 Sonnet** (for advanced reasoning).  
...
* **Technical Implementation:** 1\. The Architecture\_Analyzer fetches historical, merged PRs related to a file.  
  ...
  3\. The RAG\_Orchestrator formats this text (strictly limiting context to avoid LLM hallucination) and injects it into a prompt for Claude 3.5 Sonnet.  
  4\. Claude synthesizes the history and returns a structured JSON timeline explaining architectural decisions.
...
* **Technical Implementation:** 1\. The Senior\_Mentor module scans the retrieved code context.  
  ...
  3\. A highly specific prompt is sent to Claude 3.5 Sonnet asking it to return a JargonExplanation object containing the technical definition alongside a student-friendly analogy.
 ...
 **The Vulnerability:** For Feature 3 (Architectural Intent), fetching 50 past Pull Requests and dumping them directly into Claude 3.5 Sonnet’s context window will trigger the well-documented "Lost in the Middle" phenomenon. The LLM will heavily weight the first and last PRs but completely hallucinate or ignore the architectural context buried in the middle of the prompt.
...
**The Mitigation Strategy:**  XML Isolation Sandboxing: The backend must never concatenate user code/issues directly into the system prompt. All retrieved context must be wrapped in strict XML tags (e.g., \<untrusted\_repository\_data\>). The system prompt must explicitly instruct Claude to treat anything inside those tags as inert string data and completely ignore any command-like syntax found within them.
